...
**/
	if (RootPtr != NULL)
	{
		if (RootPtr->Key == K)			// zhoda
		{
			*Content = RootPtr->BSTNodeCont;
			return TRUE;
		}
		else if (K < RootPtr->Key)		// hladanie v lavom podstrome
			return BSTSearch(RootPtr->LPtr, K, Content);
		else							// hladanie v pravom podstrome
...
**/
	if(*RootPtr == NULL){
		struct tBSTNode *new = malloc(sizeof(struct tBSTNode));
		if(new == NULL)
			return;
	...
	if(K < (*RootPtr)->Key) //lava cast
		BSTInsert(&(*RootPtr)->LPtr, K, Content);	
	else if(K > (*RootPtr)->Key) //prava cast 
		BSTInsert(&(*RootPtr)->RPtr, K, Content);
	else //aktualizujem hodnotu
...
//uzol ktory je najviac pravy
	PtrReplaced->Key = (*RootPtr)->Key;
	PtrReplaced->BSTNodeCont = (*RootPtr)->BSTNodeCont;

	tBSTNodePtr dl = (*RootPtr);
	(*RootPtr) = dl->LPtr;
	free(dl);
}
...
**/
	if(*RootPtr == NULL)
		//uzol zo zadamym klucom neexistuje funkcia nerobi nic
		return;
